...
imageFolderThreshold = '{}\\{}'.format(imageFolder, 'thresholderHog')
imageExtension = '.jpg'
...
imageMomentsFile = 'index_hog.pkl'
imageSize = 224
imageBorderSize = 50
imageRadius = 224
zernikeDegree = 8
...
for path in imagesInFolder:
	...
	normalized = imutils.resize(original, height = 224)

	if normalized.shape[1] > 224:
		normalized = imutils.resize(normalized, width = 224)

	fd, hog_image = hog(normalized, orientations=8, pixels_per_cell=(16, 16), cells_per_block=(1, 1), visualize=True, multichannel=True)

	if imageName == '001_0002':	
		print(fd)
